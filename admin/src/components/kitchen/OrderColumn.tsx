...
const OrderColumn = ({ title, status, orders, icon: Icon, color, onStatusChange }: OrderColumnProps) => {
  ...
  return (
    <div ref={setNodeRef} className={`flex flex-col rounded-lg shadow-lg overflow-hidden ${colors.bg} border-2 ${colors.border}`}>
      {/* Header */}
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          <div className="flex items-center space-x-2">
            ...
            {isLate() && (
              <AlertCircle className="w-5 h-5 text-red-500" title="Order is late!" />
            )}
          </div>
